...
class TestList:
    def test_classify_sentiment(self):
        result = vexora.classify("This is a great feature!", sentiment)
        assert result == "Positive"

    def test_classify_negative_sentiment(self):
        result = vexora.classify(
    ...
    def test_classify_number(self):
        ...
        result = vexora.classify(0, ["letter", "number"])
    ...
    def test_classify_object(self):
        ...
        result = vexora.classify("a person in wonderland", [p1, p2, p3])
        assert result is p1


class TestEnum:
    def test_classify_bug_tag(self):
        result = vexora.classify("This is a bug", GitHubIssueTag)
        assert result == GitHubIssueTag.BUG

    def test_classify_feature_tag(self):
        result = vexora.classify("This is a great feature!", GitHubIssueTag)
        assert result == GitHubIssueTag.FEATURE

    def test_classify_enhancement_tag(self):
        result = vexora.classify("This is an enhancement", GitHubIssueTag)
        assert result == GitHubIssueTag.ENHANCEMENT

    def test_classify_docs_tag(self):
        result = vexora.classify("This is a documentation update", GitHubIssueTag)
        assert result == GitHubIssueTag.DOCS


class TestBool:
    def test_classify_true(self):
        result = vexora.classify("2+2=4", bool)
        assert result is True

    def test_classify_false(self):
        result = vexora.classify("2+2=5", bool)
...
class TestAsync:
    async def test_classify_positive_sentiment(self):
        result = await vexora.classify_async("This is a great feature!", bool)
        assert result is True
...
class TestExamples:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("I want to do an event with vexora!", "events and relations"),
