...
from vexora.agents.team import Swarm, Team
...
from vexora.utilities.types import Labels, as_classifier, is_classifier

if TYPE_CHECKING:
    from vexora.engine.end_turn import EndTurn
    from vexora.engine.handlers import AsyncHandler, Handler
...
class Task(Generic[T]):
    ...
    def get_actor(self) -> Actor:
        ...
        return self.actor or vexora.defaults.agent
    ...
    def get_tools(self) -> list[Callable[..., Any]]:
        ...
        if self.cli:
            import vexora.tools.interactive.cli

            tools.append(vexora.tools.interactive.cli.cli)
    ...
    async def run_async(
        ...
        *,
        ...
        import vexora.engine.orchestrator

        orchestrator = vexora.engine.orchestrator.Orchestrator(
            tasks=[self],
            thread=thread,
            handlers=handlers,
        )
    ...
    def get_end_turn_tools(self) -> list[type["EndTurn"]]:
        ...
        import vexora.engine.end_turn

        tools: list[type[vexora.engine.end_turn.EndTurn]] = []
        ...
        if self.plan:
            tools.append(vexora.engine.end_turn.create_plan_subtasks(parent_task=self))

        return tools

    def mark_successful_tool(self) -> type["vexora.engine.end_turn.MarkTaskSuccessful"]:
        import vexora.engine.end_turn

        return vexora.engine.end_turn.create_mark_task_successful(self)

    def mark_failed_tool(self) -> type["vexora.engine.end_turn.MarkTaskFailed"]:
        import vexora.engine.end_turn

        return vexora.engine.end_turn.create_mark_task_failed(self)

    def mark_skipped_tool(self) -> type["vexora.engine.end_turn.MarkTaskSkipped"]:
        import vexora.engine.end_turn

        return vexora.engine.end_turn.create_mark_task_skipped(self)
