...
import vexora
import vexora.agents.team
import vexora.engine.llm
from vexora.agents.actor import Actor
from vexora.agents.agent import Agent
from vexora.database import DBLLMCall
from vexora.engine.end_turn import EndTurn
from vexora.engine.events import (
    AgentEndTurnEvent,
    AgentStartTurnEvent,
    Event,
...
from vexora.engine.print_handler import PrintHandler
from vexora.instructions import get_instructions
from vexora.memory.memory import Memory
from vexora.prompts import Template
from vexora.tasks.task import Task
from vexora.thread import Thread, get_thread, message_adapter
...
class Orchestrator:
    ...
    def __post_init__(self):
        ...
        if self.handlers is None:
            if vexora.settings.enable_default_print_handler:
                self.handlers = [PrintHandler()]
            else:
                self.handlers = []

    async def handle_event(self, event: Event):
        if vexora.settings.log_events:
    ...
    async def run_once(self, actor: Actor | None = None) -> RunResult:
        ...
        all_messages = [
            vexora.engine.llm.SystemMessage(content=orchestrator_prompt),
        ] + messages
    ...
    async def run(
        ...
        if max_turns is None:
            max_turns = vexora.settings.max_agent_turns
        if max_turns is None:
            max_turns = math.inf
