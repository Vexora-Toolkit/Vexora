...
from vexora.utilities.tools import update_fn

logger = get_logger("vexora.memory")
...
class Memory:
    ...
    provider: MemoryProvider = field(
        default_factory=lambda: vexora.defaults.memory_provider,
        repr=False,
    )
    auto_use: bool = field(
        default=False,
...
def get_memory_provider(provider: str) -> MemoryProvider:
    ...
    if provider.startswith("chroma"):
        import vexora.memory.providers.chroma as chroma_provider
    ...
    # --- LanceDB ---
        ...
        import vexora.memory.providers.lance as lance_provider

        return lance_provider.LanceMemory()

    # --- Postgres ---
    elif provider.startswith("postgres"):
        import vexora.memory.providers.postgres as postgres_provider
