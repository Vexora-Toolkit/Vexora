...
import vexora
from vexora.memory.memory import MemoryProvider
...
class ChromaMemory(MemoryProvider):
    ...
    client: chromadb.ClientAPI = field(
        default_factory=lambda: chromadb.PersistentClient(
            path=str(vexora.settings.home_path / "memory/chroma"),
        ),
    )
...
def ChromaPersistentMemory(path: str | None = None, **kwargs: Any) -> ChromaMemory:
    return ChromaMemory(
        client=chromadb.PersistentClient(
            path=path or str(vexora.settings.home_path / "memory" / "chroma"),
            **kwargs,
        ),
    )
...
def ChromaCloudMemory(
    ...
    **kwargs: Any,
    ...
    tenant = tenant or vexora.settings.chroma_cloud_tenant
    assert tenant, "Tenant is required"
    database = database or vexora.settings.chroma_cloud_database
    assert database, "Database is required"
    return ChromaMemory(
        client=chromadb.CloudClient(
            api_key=api_key or vexora.settings.chroma_cloud_api_key,
            tenant=tenant,
            database=database,
            **kwargs,
        ),
