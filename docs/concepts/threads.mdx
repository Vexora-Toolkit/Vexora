...
Threads in vexora provide a powerful mechanism for managing conversation history and context across your AI workflows. Unlike ControlFlow's flows, threads in vexora are focused on conversation management and persistence rather than workflow orchestration.

## Understanding Threads

A thread in vexora represents a conversation context that can span multiple interactions. Each thread:
...
```python
thread = vexora.Thread(id="user_123_lesson")
...
from vexora.engine.llm import UserMessage, AssistantMessage
thread.add_messages([
...
vexora provides context management for threads, making it easy to set the current thread:
...
```python
current_thread = vexora.get_current_thread()
```
...
```python
parent_thread = vexora.Thread()
child_thread = vexora.Thread(parent_id=parent_thread.id)
```
...
same_thread = vexora.Thread(id=thread.id)
