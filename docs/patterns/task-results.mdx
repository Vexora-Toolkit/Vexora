...
vexora provides powerful ways to control and validate the output of tasks. By specifying a `result_type`, you can ensure that tasks return exactly the data structure you need.
...
For classification tasks, vexora provides several ways to specify options. Under the hood, vexora optimizes classification by having agents choose indices rather than writing out full labels:
...
For tasks where multiple labels can apply, use either `list[Literal]`, `list[Enum]`, or vexora's shorthand double-list syntax:
...
Use Python's type hints to specify collections. vexora supports several collection types:
...
While tuples are not directly supported by most LLM providers, vexora will attempt to coerce the result into a tuple for you:

```python
import vexora

# Get coordinates
coordinates = vexora.run(
    "Convert '40.7128° N, 74.0060° W' to decimal coordinates",
    result_type=tuple[float, float]
)

# Get name and age
person_info = vexora.run(
...
vexora supports several options for complex data structures, each with their own benefits:
