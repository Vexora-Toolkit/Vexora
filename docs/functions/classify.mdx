...
For complex classification needs, consider creating a custom task. The `classify` function is a convenient wrapper around vexora's task system - see [Tasks](/concepts/tasks) for more details.
...
async def main():
    result = await vexora.classify_async(
