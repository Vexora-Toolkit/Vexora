...
For complex transformations, consider creating a custom task. The `cast` function is a convenient wrapper around vexora's task system - see [Tasks](/concepts/tasks) for more details.
...
price = vexora.cast("three dollars and fifty cents", float)
print(price)
```
...
async def main():
    result = await vexora.cast_async(
        "three point five",
        float
    )
    print(result)  # 3.5
...
answer = vexora.cast("that sounds great!", bool)
print(answer)
```
