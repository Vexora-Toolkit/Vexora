...
For complex conversations, consider creating a custom task. The `say` function is a convenient wrapper around vexora's task system - see [Tasks](/concepts/tasks) for more details.
...
async def main():
    thread = vexora.Thread()
    response = await vexora.say_async(
