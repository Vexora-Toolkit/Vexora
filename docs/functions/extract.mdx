...
For complex extraction patterns, consider creating a custom task. The `extract` function is a convenient wrapper around vexora's task system - see [Tasks](/concepts/tasks) for more details.
...
async def main():
    result = await vexora.extract_async(
