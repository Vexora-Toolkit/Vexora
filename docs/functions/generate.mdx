...
For complex generation patterns, consider creating a custom task. The `generate` function is a convenient wrapper around vexora's task system - see [Tasks](/concepts/tasks) for more details.
...
ingredients = vexora.generate(List[str], instructions="Generate recipe ingredients")
print(ingredients)

# Generate a dictionary
menu = vexora.generate(Dict[str, float], instructions="Generate menu items with prices")
print(menu)
```
